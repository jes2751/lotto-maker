...
export async function generateMetadata({ params }: NumberDetailPageProps): Promise<Metadata> {
  ...
  if (!Number.isInteger(number) || number < 1 || number > 45) {
    return createPageMetadata({
      ...
      descriptionKo: "번호별 출현 빈도와 최근 포함 회차를 확인합니다.",
  ...
  return createPageMetadata({
    ...
    titleKo: `${number}번 번호 통계`,
    titleEn: `Number ${number} Detail`,
    descriptionKo: `${number}번 번호의 전체 출현 빈도와 최근 포함 회차를 확인합니다.`,
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-8 px-6 py-12">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="eyebrow">번호 상세</p>
          <h1 className="mt-4 text-4xl font-semibold text-white">{number}번 번호 통계</h1>
          <p className="mt-3 text-slate-300">
            전체 회차 기준 출현 빈도와 최근 10회 흐름, 최근 포함 회차를 함께 확인할 수 있습니다.
          </p>
        </div>
      ...
      <section className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        <div className="panel">
          <p className="eyebrow">전체 출현</p>
          <p className="mt-4 text-4xl font-semibold text-white">{overall.frequency}회</p>
          <p className="mt-2 text-sm text-slate-400">전체 회차 기준 출현 횟수</p>
        </div>
        <div className="panel">
          <p className="eyebrow">전체 비율</p>
          <p className="mt-4 text-4xl font-semibold text-white">{overall.percentage}%</p>
          <p className="mt-2 text-sm text-slate-400">전체 회차 기준 포함 비율</p>
        </div>
        <div className="panel">
          <p className="eyebrow">최근 10회</p>
          <p className="mt-4 text-4xl font-semibold text-white">{recent.frequency}회</p>
          <p className="mt-2 text-sm text-slate-400">최근 10회 기준 출현 횟수</p>
        </div>
        <div className="panel">
          <p className="eyebrow">최근 비율</p>
          <p className="mt-4 text-4xl font-semibold text-white">{recent.percentage}%</p>
          <p className="mt-2 text-sm text-slate-400">최근 10회 기준 포함 비율</p>
        </div>
      </section>
      ...
      <section className="panel">
        <div className="flex items-end justify-between gap-3">
          <div>
            <p className="eyebrow">최근 포함 회차</p>
            <h2 className="mt-3 text-2xl font-semibold text-white">{number}번 번호가 나온 회차</h2>
          </div>
        </div>
