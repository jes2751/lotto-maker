...
## 2026-04-22 AdSense & SEO Optimization Handoff
- **작업 목표**: 구글 애드센스 '가치 있는 인벤토리(Valuable Inventory)' 기준 충족 및 '빈약한 콘텐츠(Thin Content)' 페널티 방어.
- **콘텐츠 보강 (E-E-A-T 강화)**: 
  - 단순 로또 번호 생성기가 아닌, 수학적/통계적/법률적/재무적 가치를 제공하는 총 13개의 고품질 가이드 아티클 작성 완료 (`src/app/guides/*`).
  - 주요 주제: 명당의 진실(표본 크기), 연금복권과의 기댓값 비교, 당첨 후 자산 관리, 1등 당첨 확률의 진실, 합계 및 홀짝 패턴의 통계적 분석 등.
  - 마크다운 Bolding 문법(`**`)이 JSX에서 그대로 노출되는 문제를 해결하기 위해 `<strong className="text-white">` 태그로 일괄 변환하는 정규식 스크립트 작성 및 적용.
- **UX/UI 및 내비게이션 개편**:
  - 기존에 개별 통계 대시보드(`/stats`)로 직결되던 헤더 메뉴를 통합 메뉴인 **`[통계 허브]`(`/lotto-statistics`)**로 교체하여 하위 6개 통계 기능들의 접근성 극대화.
  - 사용자 여정을 고려한 헤더 메뉴 재배치: `생성기` → `당첨 확인` → `회차` → `통계 허브` → `유저 통계` → `가이드`.
  - 홈페이지(`/page.tsx`) 하단 위젯에 최신/인기 가이드 5종 노출 및 모든 가이드 전체 보기 링크 연결.
  - 가이드 허브(`/guides`) 내부의 끊어진 생성기 링크(`/lotto-number-generator` -> `/generate`) 복구.
- **현재 상태**: 모든 코드와 콘텐츠는 원격 저장소(`main` 브랜치)에 Push 및 배포가 진행 중이며, 배포 완료 후 애드센스 재심사 신청 대기 중 상태임.
