...
- **수동 갱신 명령어**: 로컬 터미널에서 `npm run sync:draws` 실행
- **반영 방법**: 스크립트가 공식 API에서 데이터를 가져와 `src/lib/data/local-draws.ts` 파일을 수정해주면, **변경된 로컬 파일만 Git Commit & Push** 합니다. Vercel/Cloudflare 배포가 돌면서 방문자 환경에 업데이트 적용 완료!
