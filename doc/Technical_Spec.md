...
* `필터 추첨기`를 위해 생성 API와 생성 로직에 필터 입력 구조를 확장해야 한다.
* 필터 입력은 제외수, 고정수, 홀짝 조건, 합계 범위, 연속번호 허용 여부를 우선 지원한다.
* `종합 통계 대시보드`를 위해 기본 빈도 외에 홀짝, 연속 미출현, 연속번호 계산 함수를 추가해야 한다.
* 번호 상세 통계는 최근 포함 회차 외에 기초 추이 정보까지 탐색할 수 있게 확장한다.
* 퍼블리시 전 QA에서는 모바일 레이아웃, 광고 비노출/노출 분기, 전체 회차 데이터 로딩을 함께 검증한다.
